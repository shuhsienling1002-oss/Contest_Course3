...
st.markdown("""
    ...
    /* 憶珊教練風格 */
    h1 { text-align: center; color: #880E4F !important; font-family: "Microsoft JhengHei"; margin-bottom: 20px; }
...
calendar(events=events, options={"initialView": "dayGridMonth", "headerToolbar": {"left": "prev,next", "center": "title", "right": "dayGridMonth,listMonth"}}, key="cal_fix_fem_v1")
st.divider()
        ...
        with t3: # 名單
            ...
            estu = st.data_editor(df_stu, num_rows="dynamic", use_container_width=True,
                column_config={
                    ...
                    "備註": st.column_config.TextColumn("備註 (可打字)"), # 強制文字框
                    "購買堂數": st.column_config.NumberColumn("購買堂數 (僅數字)")
                })
            if st.button("💾 更新名單"):
